...
from encoder.utils import instantiate_from_config, get_device
...
ALLJOINED_C_NUM = len(ALLJOINED_CHANNELS)
ALLJOINED_SUBJECTS = [f'sub-{i:02d}' for i in range(1, 21)]
...
def _default_subjects(config):
    data_cfg = config.get('data', {})
    if data_cfg.get('subjects'):
        return list(data_cfg['subjects'])
    dataset = config.get('dataset', 'eeg')
    if dataset == 'alljoined':
        return list(ALLJOINED_SUBJECTS)
...
def load_eeg_data(config):
    ...
    if exp_setting == 'intra-subject':
        ...
        all_subjects = _default_subjects(config)
...
class EEGDataset(Dataset):
    def __init__(self, config, mode):
        ...
        data_cfg = config.get('data', {})
        if self.mode == 'train':
            cfg_key = 'n_train_classes'
            default_n_cls = 1654
        elif self.mode == 'val':
            cfg_key = 'n_val_classes'
            default_n_cls = 200
        else:
            cfg_key = 'n_test_classes'
            default_n_cls = 200
        self.n_cls = int(data_cfg.get(cfg_key, default_n_cls))
        ...
        if self.loaded_data and self.loaded_data[0].get('ch_names') is not None:
            ...
            if self.selected_ch == config['data']['selected_ch'] or self.selected_ch == self.channels:
                self.selected_ch = self.channels
        ...
        if feature_parent:
            data_dir = os.path.join(feature_parent, f"{config['data']['blur_type']['target'].rsplit('.',1)[-1]}")
        else:
        ...
        self.image_root = data_cfg.get('image_root') or os.path.join(self.data_dir, '../Image_set_Resize')

        pretrain_map = CLIP_BACKBONES
        ...
        if  os.path.exists(features_filename):
            ...
            pretrained = resolve_clip_pretrained(
                self.model_type, pretrain_map[self.model_type]['pretrained'])
            ...
            if self.config['data']['uncertainty_aware']:
                ...
                for tag in ['low','medium','high']:
                    self.img_features[tag] = self.ImageEncoder(clip_imgs, self.blur_transform[tag])
    ...
    def ImageEncoder(self,images,blur_transform=None):
        ...
        for i in tqdm(range(0, len(set_images), batch_size)):
            ...
            ele = [self.process_transform(blur_transform(Image.open(os.path.join(self.image_root, img)).convert("RGB"))) for img in batch_images]
