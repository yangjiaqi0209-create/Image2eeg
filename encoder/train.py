...
from encoder.utils import update_config , ClipLoss, instantiate_from_config, get_device
...
class PLModel(pl.LightningModule):
    ...
    def configure_optimizers(self):
        ...
        optimizer = globals()[self.config['train']['optimizer']](
            self.parameters(), lr=self.config['train']['lr'], weight_decay=weight_decay,
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="eeg",
        help="Legacy dataset tag written into config (THINGS: eeg; Alljoined: alljoined)",
    )
    parser.add_argument(
        "--dataset-profile",
        type=str,
        default=None,
        help="Registry id (e.g. alljoined_eeg). Merges paths/dims into config; omit for THINGS-EEG.",
    )
    ...
    # Alljoined (and similar): no val.pt; load_eeg_data uses test as val_loader — still monitor val_top1.
    monitor_val = (
    ...
    elif config.get('data', {}).get('separate_val', False):
        early_stop_callback = EarlyStopping(
            monitor='val_top1_acc',
            min_delta=0.001,
            patience=es_patience,
    ...
    trainer = Trainer(log_every_n_steps=10, strategy=strategy, callbacks=[early_stop_callback, checkpoint_callback], max_epochs=config['train']['epoch'], devices=devices, accelerator='cuda', logger=logger)
    ...
    if config['exp_setting'] == 'inter-subject':
        ...
        test_results = trainer.test(ckpt_path='best' if config.get('data', {}).get('separate_val', False) else 'last', dataloaders=test_loader)
